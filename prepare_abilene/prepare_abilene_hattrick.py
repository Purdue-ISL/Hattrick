...
def dump_tms(tm, priority, path, tm_id):
    tm_file = open(f"{path}_{priority}/t{tm_id}.pkl", "wb")
...
for filename in tms_files[:8]:
    ...
    for line in file.readlines():
        ...
        high = np.random.uniform(0.45, 0.5, size=matrix.shape)
        med = np.random.uniform(0.4, 0.45, size=matrix.shape)
        low = 1 - high - med
        matrix_high = matrix*high
        matrix_mid = matrix*med
        matrix_low = matrix*low
        for i, tm in zip([1, 2, 3], [matrix_high, matrix_mid, matrix_low]):
            dump_tms(tm, i, path_tms, tm_id)
